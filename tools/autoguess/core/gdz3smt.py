...
class ReduceGDtoZ3SMT:
    ...
    def __init__(
        ...
        self.supported_sat_solvers = (
            list(solvers.SolverNames.cadical)
    ...
    def generate_initial_conditions(self):
        ...
        initial_state_vars = list(
            map(
                lambda x: (z3.Bool(x), 1),
                [step_var(v, 0) for v in self.variables if v not in _known_set],
            )
        )
        if initial_state_vars != []:
            self.solver.add(
                z3.PbLe(
                    [initial_state_vars[i] for i in range(len(initial_state_vars))],
                    self.max_guess,
                )
            )
    ...
    def make_model(self):
        ...
        file_path = os.path.join(
            TEMP_DIR, "smtmodel_mg%d_ms%d.smt2" % (self.max_guess, self.max_steps)
        )
        ...
        if self.log == 1:
            ...
            print(
                "SMT model was generated after %0.2f seconds and was written into %s"
    ...
    def convert_smt_to_cnf(self):
        ...
        if self.log == 1:
            print(
                "SMT was converted to the CNF after %0.2f seconds and was written into %s"
    ...
    def solve_via_satsolver(self):
        ...
        if self.sat_solver_name in solvers.SolverNames.cadical:
            ...
            print(
                "Choose a solver from the following list please:%s"
        ...
        if self.time_limit != -1:
            if self.sat_solver_name in list(solvers.SolverNames.cadical) + list(
                solvers.SolverNames.lingeling
            ):
        ...
        if result == True:
            ...
            for step in range(self.max_steps + 1):
                ...
                state_values = list(
                    map(
                        lambda x: int(
                            self.satsolver_solution[self.dimacs_vars_dict[x] - 1] > 0
                        ),
    ...
    def solve_via_smtsolver(self):
        ...
        print(
            "Checking the satisfiablity of the obtained SMT model using z3-solver ..."
        )
        if self.time_limit != -1:
            ...
            for step in range(self.max_steps + 1):
                ...
                state_values = list(
                    map(
                        int,
                        [
                            z3.is_true(self.solver_model[st_var])
                            for st_var in z3_state_vars
                        ],
                    )
                )
                self.solutions[step] = dict(zip(state_vars, state_values))
