...
def draw_graph(
    vertices, edges, known_variables, guessed_vars, output_dir, tikz, dglayout
):
    """
    ...
    for edge in edges:
        ...
        node1_name = (
            "{%s_{%s}}" % (edge[0].split("_")[0], ",".join(edge[0].split("_")[1:]))
            if "_" in edge[0]
            else edge[0]
        )
        node2_name = (
            "{%s_{%s}}" % (edge[1].split("_")[0], ",".join(edge[1].split("_")[1:]))
            if "_" in edge[1]
            else edge[1]
        )
    ...
    for vertex in vertices:
        vertex_name = (
            "{%s_{%s}}" % (vertex.split("_")[0], ",".join(vertex.split("_")[1:]))
            if "_" in vertex
            else vertex
        )
        color, shape = (
            ...
            if vertex in known_variables
                ...
                ("red", "doublecircle")
                if vertex in guessed_vars
                else ("green", "circle")
            )
        )
        directed_graph.node(vertex, color=color, style="filled", shape=shape)
        digraph_tikz.node(
            vertex_name, color=color, style="filled", shape=shape, texmode="math"
        )
