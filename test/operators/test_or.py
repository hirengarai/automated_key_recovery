...
from operators.boolean_operators import OR
...
def gen_operator(bitsize=2):
    ...
    op = OR(my_input, my_output, ID='OR')
    op.display()
    return op
...
def trans_sbox():
    ...
    or_sbox = Sbox([var.Variable(2,ID="in")], [var.Variable(1,ID="out")], input_bitsize=2, output_bitsize=1, ID="or_sbox")
