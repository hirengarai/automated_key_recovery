...
def gen_operator(bitsize, n):
    print("\n********************* operation: N_XOR ********************* ")
    my_input, my_output = [var.Variable(bitsize,ID="in"+str(i)) for i in range(n+1)], [var.Variable(bitsize,ID="out")]
    op = N_XOR(my_input, my_output, ID = str(n)+'N_XOR')
    op.display()
    return op
...
def test_milp_model(op):
    model_versions = [op.__class__.__name__+"_XORDIFF", op.__class__.__name__+"_XORDIFF_1", op.__class__.__name__+"_LINEAR", op.__class__.__name__ + "_TRUNCATEDLINEAR"]
...
def test_nxor(bitsize, n):

    op = gen_operator(bitsize=bitsize, n=n)
...
if __name__ == '__main__':
    ...
    test_nxor(bitsize=1, n=2)

    test_nxor(bitsize=2, n=2)
