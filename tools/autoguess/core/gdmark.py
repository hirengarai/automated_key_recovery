...
class Mark:
    ...
    def __init__(
        ...
        ), "Marking algorithm doesn't support implication relations"
    ...
    def generate_and_triangulate_dependency_matrix(self):
        ...
        rows = [
            {"relation": rel, "weight": len(rel)} for rel in self.symmetric_relations
        ]
        rows = sorted(rows, key=itemgetter("weight"), reverse=True)
        columns = [
            {
                "variable_name": var,
                "coefficients": [int(var in row["relation"]) for row in rows],
            }
    ...
    def find_minimal_guess_basis(self):
        ...
        self.guess_basis = []
        determined_target_variables = [
            v
            for v in self.target_variables
            if self.dependency_matrix_columns[self.get_index(v)]["known"] == 1
        ]
        i = 1
        while i <= self.num_of_relations and len(determined_target_variables) != len(
            ...
            unknown_vars_indices = [
                ind
                for ind in relation_vars_indices
                if self.dependency_matrix_columns[ind]["known"] == 0
            ]
            ...
            if len(unknown_vars_indices) == len(relation_vars):
                ...
                self.guess_basis.extend(
                    [
                        self.dependency_matrix_columns[j]["variable_name"]
                        for j in unknown_vars_indices[0:num_of_guesses]
                    ]
                )
